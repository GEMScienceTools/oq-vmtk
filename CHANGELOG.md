...
- `test_modeller.TestModellerMethods.test_modal_analysis_mode_shape`: compared the raw mode shape against the expected `PHI` after normalising only for scale, not sign — eigenvectors are only defined up to a sign, and different OpenSeesPy builds (e.g. Windows `openseespy` vs. macOS `openseespymac`) can return `+phi` or `-phi` for the same mode, causing a spurious failure (`[-0.6514, -1.0]` vs. expected `[0.6514, 1.0]`) on platforms other than the one the test was authored against. The test now canonicalises the sign (flips so the top-storey component is always positive) before comparing.
