...
This folder contains an ensemble of example python notebooks demonstrating various steps within the GEM global vulnerability workflow highlighting earthquake catastrophe modelling and visualization techniques. Below is an overview of the available examples:
...
## IntensityMeasureSelection: Example of Intensity Measure Selection
This example provides a framework for selecting optimal intensity measures (IMs) for seismic demand modelling. Four IM selection metrics are evaluated — Efficiency, Proficiency, Practicality, and the Relative Sufficiency Measure (RSM) — across both Modified Cloud Analysis and Incremental Dynamic Analysis results.
...
## ModalAnalysis: Example of Modal Analysis
This example provides a workflow for developing and verifying simplified structural models in OpenSees through modal analysis. The MDOF stick-and-mass model is calibrated from an SDOF capacity curve and its dynamic properties — natural periods, mode shapes, and modal participation factors — are extracted using the `modeller` module.
...
## ModifiedCloudAnalysis: Example of End-to-End Vulnerability Analysis of an MDOF System using Cloud Analysis
...
## MultipleStripeAnalysis: Example of End-to-End Vulnerability Analysis using Multiple Stripe Analysis
This example performs nonlinear time-history analysis on a multi-degree-of-freedom structural system using Multiple Stripe Analysis (MSA), where ground motions are organised into hazard-consistent intensity-based stripes. It demonstrates fragility derivation via Maximum Likelihood Estimation and vulnerability assessment from fragility and consequence models, using the `calibration`, `modeller`, `postprocessor`, and `plotter` modules.

## IncrementalDynamicAnalysis: Example of End-to-End Vulnerability Analysis using Incremental Dynamic Analysis
This example performs nonlinear time-history analysis on a multi-degree-of-freedom structural system using the Hunt, Trace and Fill algorithm to scale ground motion records to collapse. It demonstrates IDA-based fragility derivation and vulnerability assessment, using the `calibration`, `modeller`, `postprocessor`, and `plotter` modules.

## FragilityAnalysis: Example of Supported Fragility Analyses Methods
This example demonstrates the application of the `postprocessor` module to derive fragility functions through both conventional and state-of-the-art methodologies. It applies multiple approaches to estimate probabilities of exceedance for arbitrary demand-based damage states following nonlinear time-history analyses. The methods include lognormal cumulative distribution functions, generalized linear models, and ordinal models. The treatment of additional epistemic uncertainty in fragility functions is also addressed.
