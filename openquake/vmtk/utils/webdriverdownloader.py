...
class WebDriverDownloaderBase:
    ...
    def download(self, version="latest", os_name=None, bitness=None, show_progress_bar=True):
        ...
        if os.path.isfile(filename_with_path):
            logger.info(f"Skipping download. File {filename_with_path} already exists.")
            return filename_with_path
        
        # Start downloading
        data = requests.get(download_url, stream=True)
        if data.status_code == 200:
            logger.debug(f"Starting download of {download_url} to {filename_with_path}")
            ...
            logger.debug(f"Finished downloading {download_url} to {filename_with_path}")
            return filename_with_path
        
        else:
            error_message = f"Error downloading file {filename}, got status code: {data.status_code}"
